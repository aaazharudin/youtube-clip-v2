...
const I18N = {
  id: {
    "top.tagline": "Ubah video YouTube jadi klip viral dalam sekejap.",
  ...
  en: {
    "top.tagline": "Convert YouTube videos to viral clips instantly.",
    "label.url": "YouTube URL",
    "ph.url": "https://www.youtube.com/watch?v=...",
...
function getStageName(stage) {
  const stages = {
    ...
    subtitle_model_load: "🤖 Load AI model",
    subtitle_transcribe: "🎤 Transcribe audio",
    subtitle_write: "✍️ Tulis subtitle",
    burn_subtitle: "🔥 Burn subtitle ke video",
    finalize: "🎬 Finalizing video",
    generating_title: "✨ Generate judul AI",
    done_clip: "✅ Selesai"
  };
  return stages[stage] || stage || "⏳ Memproses...";
}
...
function readPayload() {
  ...
  // Get title mode
  const titleMode = document.querySelector('input[name="title_mode"]:checked')?.value || "auto";

  // Parse manual titles if in manual mode
  const customTitles = {};
  if (titleMode === "manual") {
    const manualTitlesText = ($("manual_titles")?.value || "").trim();
    if (manualTitlesText) {
      const lines = manualTitlesText.split("\n").filter(l => l.trim());
      lines.forEach((line, idx) => {
        if (line.trim()) {
          customTitles[String(idx + 1)] = line.trim();
        }
      });
    }
  }
...
function renderProgress(job) {
  ...
  // Clear status display - more user friendly
  let statusHTML = "";
  let progressText = "";

  if (job.status === "done") {
    statusHTML = `<span class="status-done">✅ Selesai!</span>`;
    progressText = `${job.success || 0} dari ${total} klip berhasil dibuat`;
  } else if (job.status === "error") {
    statusHTML = `<span class="status-error">❌ Gagal</span>`;
    progressText = job.error || "Terjadi kesalahan";
  } else if (job.status === "running") {
    const stageName = getStageName(job.stage);
    statusHTML = `<span class="status-running">⏳ ${stageName}</span>`;
    progressText = `Klip ${done} dari ${total}`;
  } else if (job.status === "queued") {
    statusHTML = `<span class="status-queued">⏸ Menunggu...</span>`;
    progressText = "Antrian diproses";
  }
  ...
  if (job.status === "error") {
    ...
    err.textContent = job.error || "Terjadi kesalahan";
    root.appendChild(err);
  }

  if (Array.isArray(job.outputs) && job.outputs.length > 0) {
    // Add "View Gallery" button when done
    if (job.status === "done") {
      const galleryBtn = document.createElement("div");
      galleryBtn.className = "galleryBtn";
      galleryBtn.innerHTML = `
        <a href="/gallery" class="btn" style="width:100%;text-align:center;background:linear-gradient(135deg, #06b6d4, #14b8a6);">
          📬 Lihat Semua Klip di Gallery
        </a>
      `;
    ...
    job.outputs.forEach((f) => {
      ...
      el.innerHTML = `
        <div class="outLeft">
          <a href="${href}" target="_blank" rel="noreferrer" class="out-link">${f.name}</a>
          <div class="small">${Math.round((f.size || 0) / 1024)} KB</div>
        </div>
        <div class="outRight">
          <a href="${href}" target="_blank" class="btn ghost smallBtn" rel="noreferrer">▶️ Play</a>
          <a class="btn smallBtn" href="${href}" download>⬇️ Download</a>
        </div>
      `;
...
function toggleTitleMode() {
  const selectedMode = document.querySelector('input[name="title_mode"]:checked')?.value || "auto";
  const manualBox = $("manualTitlesBox");
  if (manualBox) {
    manualBox.classList.toggle("hide", selectedMode !== "manual");
  }
}
...
document.querySelectorAll('input[name="title_mode"]').forEach(radio => {
  radio.addEventListener("change", toggleTitleMode);
});
