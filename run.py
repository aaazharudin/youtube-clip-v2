...
# Thread-local config context variable
_config_context: ContextVar = ContextVar("_config_context", default=None)


@dataclass
class ClipConfig:
    """Configuration for clip processing (thread-safe)."""
    ...
    def get_dimensions(self):
        """Get output dimensions based on ratio."""
        if self.output_ratio == "9:16":
            return 720, 1280
        elif self.output_ratio == "1:1":
            return 720, 720
        elif self.output_ratio == "16:9":
            return 1280, 720
        elif self.output_ratio == "original":
            return None, None
        else:
            raise ValueError(f"Invalid ratio preset: {self.output_ratio}")


def get_config() -> ClipConfig:
    """Get current thread-local config."""
    config = _config_context.get()
    if config is None:
        # Return default config if not set (for CLI usage)
        return ClipConfig()
    return config


def set_config(config: ClipConfig):
    """Set thread-local config."""
    _config_context.set(config)


# Legacy global variables (kept for backward compatibility with CLI)
...
def build_subtitle_force_style(config=None):
    if config is None:
        config = get_config()
...
def generate_subtitle(video_file, subtitle_file, event_hook=None, config=None):
    """
...
def proses_satu_clip(video_id, item, index, total_duration, crop_mode="default", use_subtitle=False, event_hook=None, title_mode="auto", custom_title=None, source_metadata=None):
    """
    ...
    try:
        ...
        out_w, out_h = config.get_dimensions()
        ...
        if use_subtitle:
            ...
            if generate_subtitle(cropped_file, subtitle_file, event_hook=event_hook, config=config):
                if callable(event_hook):
                    try:
                ...
                force_style = build_subtitle_force_style(config)
                cmd_subtitle = [
                    "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
                    "-i", cropped_file,
        ...
        if title_mode == "auto":
            if callable(event_hook):
                try:
                    event_hook("stage", {"stage": "generating_title", "clip_index": index})
                except Exception:
                    pass
            print("  Generating AI title...")
            try:
                from ai_title_generator import ClickbaitAI
                ai = ClickbaitAI()
                # Use source metadata for context if available
                context = ""
                if source_metadata:
                    if source_metadata.get("title"):
                        context += f"Video: {source_metadata['title']}\n"
                    if source_metadata.get("channel"):
                        context += f"Channel: {source_metadata['channel']}\n"

                clip_title = ai.generate_title(
                    video_path=output_file,
                    source_context=context if context else None
                )
                if clip_title:
                    print(f"  ✅ AI Title: {clip_title}")
                else:
                    print("  ⚠️  AI title generation failed, using default")
                    clip_title = f"Clip #{index} - Viral Moment 🔥"
            except Exception as e:
